...
class TplinkBase(RouterBase):
    ...
    def _get_status_array(self, array_name):
        ...
        return self._extract_js_array_data(contents, array_name)

    def get_router_info(self):
        return self._parse_router_info(self._get_status_array('statusPara'))

    def get_mac_address(self):
        return self._parse_mac_address(self._get_status_array('wanPara'))

    def get_dns_servers(self):
        return self._parse_dns_servers(self._get_status_array('wanPara'))

    def get_pppoe_online_time(self):
        return self._parse_pppoe_online_time(self._get_status_array('wanPara'))

    def get_uptime(self):
        return self._parse_uptime(self._get_status_array('statusPara'))

    def get_traffic_stats(self):
        return self._parse_traffic_stats(self._get_status_array('statistList'))

    def get_connected_clients_list(self):
        return self._parse_connected_clients_list(self._make_http_request_read('AssignedIpAddrListRpm.htm'))

    def get_dmz_settings(self):
        return self._parse_dmz_settings(self._make_http_request_read('DMZRpm.htm'))

    def push_dmz_settings(self, settings):
        get_params = self._generate_dmz_data(settings)
        contents = self._make_http_request_write('DMZRpm.htm?%s' % urllib.urlencode(get_params))
        return self._parse_dmz_settings(contents) == settings

    def get_addr_reservation_list(self):
        return self._parse_addr_reservation_list(self._make_http_request_read('FixMapCfgRpm.htm'))
    ...
    def push_addr_reservation_list(self, lst_new):
        ...
        for item in lst_new:
            data = self._generate_addr_reservation_item_data(item)
    ...
    def get_dhcp_settings(self):
        return self._parse_dhcp_settings(self._make_http_request_read('LanDhcpServerRpm.htm'))
    ...
    @classmethod
    def _parse_pppoe_online_time(cls, data_array):
        try:
            ...
            return cls._parse_uptime_to_seconds(uptime_string)
    ...
    @classmethod
    def _parse_dmz_settings(cls, contents):
        array_name = 'DMZInf'

        result = cls._extract_js_array_data(contents, array_name)
    ...
    @classmethod
    def _parse_addr_reservation_list(cls, contents):
        array_name = 'dhcpList'
    ...
    @classmethod
    def _parse_connected_clients_list(cls, html):
        # the last 2 elements of the data array are not needed
        result = cls._extract_js_array_data(html, 'DHCPDynList')[:-2]
        ...
        for client_name, mac, ip, lease_time in split_in_groups(result, 4):
            ...
            if lease_time == 'Permanent':
                ...
                item.set_lease_time(cls._parse_lease_time(lease_time))
    ...
    @classmethod
    def _parse_dhcp_settings(cls, html):
        settings = DHCPServerSettings()

        array = cls._extract_js_array_data(html, 'DHCPPara')
    ...
    @classmethod
    def _generate_addr_reservation_item_data(cls, item):
        data = {}
        data['Mac'] = cls._denormalize_mac(item.mac)
        data['Ip'] = item.ip
